    ...
    def inference(self, memory):
        ...
        while True:
            ...
            if torch.sigmoid(gate_output.data) > self.gate_threshold:
                ...
                logger.warn(f"Reached max decoder steps - {self.max_decoder_steps}")
                break
...
def load_model(data: Union[str, io.IOBase, os.PathLike], gate_threshold=0.1, gpu=True):
    params = create_params()
    params.gate_threshold = gate_threshold
    if gpu:
        device = torch.device("cuda")
        model = Tacotron2(params).cuda()
    else:
    ...
    if gpu:
        ...
        model.load_state_dict(torch.load(data, map_location=device)["state_dict"])
        model.eval().float()
